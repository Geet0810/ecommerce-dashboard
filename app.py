...
This version does lazy Plotly imports and displays a helpful error in Streamlit
if Plotly is missing or fails to import (so the app doesn't crash at import time).
"""
...
def get_plotly():
    """
    Lazy import Plotly. If Plotly cannot be imported, show a Streamlit error
    with installation instructions and stop execution.
    """
    try:
        ...
        st.error(
            "Plotly is required to render charts but failed to import.\n\n"
            "Please install Plotly in your environment and restart the app:\n\n"
            "pip install plotly\n\n"
            f"Error details: {e}"
        )
        st.stop()
